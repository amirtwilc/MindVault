...
object ReminderPlugin {
    ...
    fun schedule(
        ...
        val pi = alarmServicePendingIntent(context, noteId, title, body, remindAtMillis, version)
        ...
        val strategy = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            alarmManager.setAlarmClock(
                AlarmManager.AlarmClockInfo(fireAt, notificationTapPendingIntent(context, noteId)),
                pi
            )
            "alarm_clock"
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && exactAllowed) {
    ...
    fun buildNotification(
        ...
        if (!notificationsAllowed(context)) {
            Log.w(TAG, "Skipping reminder notification because permission is missing noteId=$noteId")
            return null
        }
        val tapPi = notificationTapPendingIntent(context, noteId)
