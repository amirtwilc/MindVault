...
void main() {
  ...
  group('updateNote', () {
    ...
    test('updates Drift immediately', () async {
      when(() => remote.updateNote(any(), any())).thenAnswer((_) async => _fakeModel(
    ...
    test('queues update_note pending op when Supabase fails', () async {
      when(() => remote.updateNote(any(), any())).thenThrow(Exception('offline'));
  ...
  group('deleteNote', () {
    ...
    test('queues delete_note pending op when Supabase fails', () async {
      ...
      await repo.deleteNote('note-1');
      final ops = await db.getPendingOps();
      expect(ops.any((o) => o.opType == 'delete_note' && o.recordId == 'note-1'), isTrue);
    });
    ...
    test('removes AI history entries citing the deleted note', () async {
      ...
      expect(history.any((e) => e.citedNoteIdsJson.contains('note-1')), isFalse);
    });
  });
  ...
  group('syncPendingOps', () {
    ...
    test('retries create_note op on second call after first call failed', () async {
    ...
    test('delete_note op removes note from Drift even if Realtime re-inserted it', () async {
      ...
      await db.upsertPendingOp('del_note-deleted', 'delete_note', 'note-deleted');
      ...
      expect(await db.getNote('note-deleted'), isNull,
          reason: 'note must be gone from Drift after delete_note op is processed');
    });
    ...
    test('skips stale update_note op when remote already has a newer version', () async {
    ...
    test('cleans up update_note pending op when local note no longer exists', () async {
      ...
      verifyNever(() => remote.upsertNote(any()));
      expect(await db.getPendingOps(), isEmpty,
          reason: 'orphaned pending op for a locally-deleted note must be cleared');
    });

  // ── _syncAllNotes (via syncPendingOps) ────────────────────────────────────

  group('_syncAllNotes reconciliation', () {
    test('removes Drift note absent from Supabase with no pending create op', () async {
    ...
    test('does NOT remove Drift note that has a pending create_note op', () async {
      ...
      when(() => remote.upsertNote(any())).thenThrow(Exception('still offline'));
      when(() => remote.fetchAllNotes()).thenAnswer((_) async => []);
  ...
  group('setNotePinned', () {
    ...
    test('pinning sets isPinned=true and pinOrder=0 when no pinned notes exist', () async {
      when(() => remote.updateNote(any(), any())).thenAnswer((_) async => _fakeModel(
    ...
    test('queues update_note pending op when remote fails', () async {
      ...
      final ops = await db.getPendingOps();
      expect(ops.any((o) => o.opType == 'update_note' && o.recordId == 'note-1'), isTrue);
    });
    ...
    test('setNotePinned bumps updatedAt so offline changes participate in last-write-wins', () async {
      ...
      expect(row!.updatedAt.isAfter(old), isTrue,
          reason: 'updatedAt must be refreshed so the pending op carries a current timestamp');
    });
  });
  ...
  group('reorderPinnedNotes', () {
    ...
    test('queues update_note ops for each pinned note when remote fails', () async {
    ...
    test('reorderPinnedNotes bumps updatedAt so offline reorders participate in last-write-wins', () async {
  ...
  group('analytics tracking', () {
    ...
    test('createNote fires note_created event', () async {
      ...
      verify(() => analytics.track('note_created', metadata: any(named: 'metadata')))
          .called(1);
    });

    test('createNote fires note_created even when Supabase is offline', () async {
      when(() => remote.insertNote(any())).thenThrow(Exception('network error'));
    ...
    test('deleteNote fires note_deleted event', () async {
      ...
      verify(() => analytics.track('note_deleted', metadata: any(named: 'metadata')))
          .called(1);
    });
    ...
    test('analytics is optional — null analytics does not throw', () async {
      ...
      await expectLater(
        repo.createNote(categoryId: 'cat-1', title: 'T', body: 'B', isPrivate: false),
        completes,
      );
    });
  });
